...
def library_preparation(info):
    ...
    for index in gene_data.index:
        ...
        for interval in gene_exon.steps():
            ...
            if interval[1] == 0: 
                if interval[0].is_contained_in(gene_data.ix[index,"genomic_interval"]):
                    gene_all_intron.append(interval[0]) 

        gene_data.set_value(index,"exon_specific",gene_all_exon)
        gene_data.set_value(index,"introns_all",gene_all_intron)
        
    print '\t- Calculate exon/intron genes ratio'
    
    for index in gene_data.index():
        exons_size = 0
        introns_size = 0
        for interval in gene_data.ix[index,'exon_specific']:
            exons_size +=interval.length
        for interval in gene_data.ix[index,'introns_all']:
            introns_size +=interval.length
        gene_data.ix[index,'ExonVsIntron'] = float(exon_size)/introns_size
